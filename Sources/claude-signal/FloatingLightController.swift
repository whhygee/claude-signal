...
final class TrafficLightView: NSView {
    ...
    override func draw(_ dirtyRect: NSRect) {
        let housing = NSBezierPath(roundedRect: bounds.insetBy(dx: 1, dy: 1), xRadius: 14, yRadius: 14)
        ...
        let totalHeight = CGFloat(lamps.count) * diameter + CGFloat(lamps.count - 1) * spacing
        var y = bounds.midY + totalHeight / 2 - diameter // top lamp first (red on top)

        for (state, color) in lamps {
            let rect = NSRect(x: bounds.midX - diameter / 2, y: y, width: diameter, height: diameter)
